## 🚀 V1.0 Implementation Guide: Orchestrated Data Cleaning & Loading
...
- **Small Batch** (≤10k records): Routes to `small-batch-queue` for fast path processing
...
### 2. Azure Data Factory Pipeline - Medallion Architecture (Bronze → Silver → Gold)
...
3. **Transform to Silver Layer**
   ...
   - Operations:
     ...
     - Calculate data quality scores (0-100)
...
2. **ForEach Pending Job**
   - Concurrency: 3–5 (tune per cluster size)
   - Items: Jobs returned from step 1

   **Inside ForEach:**
   - **Read Bronze + Bronze → Silver (Databricks Notebook)**
     - Notebook: `/Shared/Notebooks/bronze_to_silver_pyspark`
     - Source: `shanlee-raw-data/{userId}/{parentJobId}/{jobId}.json`
     - Ops: distributed read, validation, dedupe, standardize dates/phones/emails, fill missing, quality scoring
     - Output: `silver/cleaned/{userId}/{parentJobId}/{jobId}.parquet`

   - **Silver → Gold (Databricks Notebook)**
...
### Queues
```
✓ small-batch-queue (created automatically by code)
✓ large-batch-queue (created automatically by code)
```

### Storage Accounts
```
✓ Existing: Azure Blob Storage (shanlee-raw-data container) - Bronze Layer
  - Purpose: Raw data source (READ ONLY)
  - No duplication needed - already populated by generation step
  
✓ Needed: ADLS Gen2 for Silver and Gold layers
  - Containers:
...
✓ Azure Synapse Analytics (data warehouse) - NEW
  - Tables:
...
✓ Azure Databricks (Standard cluster)
```
...
- Track Bronze (READ) → Silver (WRITE) → Gold (WRITE) progress
- Send real-time updates to frontend
- Monitor data quality scores and transformation metrics
...
├────────────────────────────────────────────────────────────┤
│  /clean_data endpoint                                      │
│  ├─> DataRouter (routing_logic.py)                        │
│  └─> Sends to appropriate Azure Queue                     │
└────────────────────────────────────────────────────────────┘
                            │
                ┌───────────┴───────────┐
                │                       │
        ...
        │  ├─ 10-min trigger (small)          │
        │  └─ 10-hour trigger (large, queue check) │
        └───────┬────────────────────────┬─────┘
                │                        │
        ┌───────▼──────┐        ┌────────▼──────┐
        │ Azure Func   │        │  Databricks   │
        │ (Pandas)     │        │  (PySpark)    │
        └───────┬──────┘        └────────┬──────┘
                │                        │
        ┌───────▼────────────────────────┴─────┐
        │   Medallion Architecture Layers      │
        │   ├─ Bronze: shanlee-raw-data (READ) │
        │   ├─ Silver: silver/cleaned (WRITE)  │
        │   └─ Gold: gold/analytics (WRITE)    │
        │   Note: No Bronze duplication        │
        └───────┬────────────────────────┬─────┘
                │                        │
        ┌───────▼──────┐        ┌────────▼──────┐
        │  Azure       │        │  Azure MySQL  │
...
2. ADF pipeline execution time (Bronze → Silver → Gold)
3. Data quality score (% cleaned in Silver)
4. Load success rate (records loaded vs. total)
5. End-to-end latency (submission → completion)
